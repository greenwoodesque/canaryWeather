...
class GeoLocationController: NSObject {
    ...
    required init(delegate: GeoLocationControllerDelegate) {
        ...
        locationManager.distanceFilter = 40000
...
extension GeoLocationController: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
        //This function is called upon initialization of CLLocationManager regardless of changed authorization. Here we ignore that initial call.
        if initialAuthorizationStatus != .authorizedAlways && initialAuthorizationStatus != .authorizedWhenInUse {
            start()
